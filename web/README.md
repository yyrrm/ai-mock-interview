...
| API | 설명 |
|---|---|
| `POST /api/resume` | 이력서 PDF 업로드 → 텍스트 추출 + 요약 + 첫 질문 |
| `POST /api/question` | 이력서 + 직전 답변 기반 다음 질문 생성 |

## 데모 흐름
1. **회원가입** → 아이디/비밀번호/닉네임 입력
2. **로그인**
3. **마이페이지**에서 `새 면접 시작`
4. **이력서(PDF) 업로드** → AI가 이력서를 분석해 맞춤 첫 질문 생성
   (이력서 없이 시작도 가능)
5. 면접 화면에서 답변 메모를 적고 `다음 질문` → 이력서·답변 기반으로 새 질문 생성
