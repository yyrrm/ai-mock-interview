...
def run_module(video_path, detector=None, frame_interval=30, display=False):
    ...
    global emotion_buffer, au_buffer
        ...
        if frame_count % frame_interval == 0:
            ...
            try:
                if result is not None:
                    ...
                    # *Detector 호출, 단순 감정수치 및 AU 기반 분석 parameter에 이 변수 사용
                    fex_detect_result = detector.detect_image("frame.jpg")

                    # 감정 수치 감지 함수 호출, 감정 수치 값 dict 반환
                    emotion_result = emotion_detect(fex_detect_result)

                    # AU 기반 분석용 AU 데이터 수집 함수 호출--------------------
                    au_result = au_collect(fex_detect_result)
                    if au_result is not None:
                        # 전역으로 선언한 list에 저장
                        au_buffer.append(au_result)
                    #----------------------------------------
    ...
    # 루프 종료 후, AU 기반 점수 출력 및 시각화 함수 출력
    au_score = finalize_au_score()
    if au_score is not None:
        au_score_total = au_score["total_expression_score"]
        print("분야별 AU 점수: ", au_score)
        print("최종 AU 점수: ", au_score_total)
        print(expression_text_feedback(au_score_total))
    else:
        print("AU 점수 데이터 없음/오류 발생")

    visualize_emo_data(list_for_emo_plot)


# 실제 Detector 함수
def emotion_detect(fex):
...
def au_collect(fex):
    ...
    au_dict = {}
    if fex is not None and not fex.empty:
        for au in used_au_list:
            if au in fex.columns:
                au_dict[au] = float(fex[au].iloc[0])
            else:
                au_dict[au] = 0.0
        return au_dict
    else:
...
def finalize_au_score():
    if len(au_buffer) == 0:
        return None

    df_au = pd.DataFrame(au_buffer)
    return calc_total_expression_score(df_au)
#---------------------------------------------------------------------
# 이동평균 계산용 임시 리스트
emotion_buffer = []
